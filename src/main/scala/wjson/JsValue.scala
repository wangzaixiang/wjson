...
given JsValueMapper[BigDecimal] with
  def fromJson(js: JsValue): BigDecimal = js match
    ...
    case _ => throw new Exception("Expected JsNumber")
  override def toJson(t: BigDecimal): JsValue = JsNumber(t.doubleValue)
...
given jmBigDecimal: JsValueMapper[java.math.BigDecimal] with
  def fromJson(js: JsValue): java.math.BigDecimal = js match
    ...
    case _ => throw new Exception("Expected JsNumber")
  override def toJson(t: java.math.BigDecimal): JsValue = JsNumber(t.doubleValue)
...
given JsValueMapper[String] with
  override def fromJson(js: JsValue): String = js match
...
given [T:JsValueMapper]: JsValueMapper[Seq[T]] with
  override def fromJson(js: JsValue): Seq[T] = js match
    case JsArray(value) => value.map(summon[JsValueMapper[T]].fromJson)
    case _ => throw new Exception("Expected JsArr")
  override def toJson(t: Seq[T]): JsValue = JsArray(t)

given [T: JsValueMapper]: JsValueMapper[List[T]] with
  override def fromJson(js: JsValue): List[T] = js match
    case JsArray(value) => value.map(summon[JsValueMapper[T]].fromJson).toList
    case _ => throw new Exception("Expected JsArr")
  override def toJson(t: List[T]): JsValue = JsArray(t.map(summon[JsValueMapper[T]].toJson))
...
given [T: JsValueMapper]: JsValueMapper[Option[T]] with
  override def fromJson(js: JsValue): Option[T] = js match
    case JsNull => None
    case _ => Some(summon[JsValueMapper[T]].fromJson(js))
  override def toJson(t: Option[T]): JsValue = t match
    case Some(x) => summon[JsValueMapper[T]].toJson(x)
    case None => JsNull

given [T: JsValueMapper]: Conversion[T, JsValue] with
  override def apply(x: T): JsValue = summon[JsValueMapper[T]].toJson(x)
...
//private case class CaseField[T: JsValueMapper](name: String, default: Option[T] = None):
//  inline def apply(js: JsObject): T = js.fields.get(name) match
//    case Some(value) => summon[JsValueMapper[T]].fromJson(value)
//    case None => default.getOrElse(throw new Exception("Expected field " + name))

inline def caseFieldGet[T: JsValueMapper](inline js: JsObject, inline name: String): T =
  js.fields.get(name) match
    case Some(value) => summon[JsValueMapper[T]].fromJson(value)
    case None => throw new Exception("Expected field " + name)

inline def caseFieldGet[T: JsValueMapper](inline js: JsObject, inline name: String, inline default:T): T =
  js.fields.get(name) match
    case Some(value) => summon[JsValueMapper[T]].fromJson(value)
    case None => default
